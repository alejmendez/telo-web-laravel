...
class UpdateRequest extends FormRequest
{
    ...
    {
        return [
            'dni' => 'required|max:20|unique:customers,dni,' . $this->id,
            'first_name' => 'required|max:100',
            'last_name' => 'required|max:100',
            'email' => 'required|email|max:200|unique:customers,email,' . $this->id,
            'phone_e164' => 'required|max:20|unique:customers,phone_e164,' . $this->id . '|regex:/^\+[1-9]\d{7,14}$/',
            'customer_type_id' => 'required|integer|exists:customer_types,id',
            'dni_location_id' => 'required|integer|exists:locations,id',
    ...
    public function attributes()
    {
        return [
            ...
            'dni_location_id' => __('customer.form.dni_location_id.label'),
    ...
    {
        $dni_location_id = is_array($this->dni_location_id) ? array_key_first($this->dni_location_id) : null;
        ...
        $this->merge([
            ...
            'dni_location_id' => $dni_location_id,
            'notes' => $this->notes,
        ]);
    }
}
