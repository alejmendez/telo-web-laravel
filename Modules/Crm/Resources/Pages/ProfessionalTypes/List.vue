...
<template>
  <AuthenticatedLayout :title="__('professionaltype.titles.entity_breadcrumb')">
    ...
    <Datatable
      ...
      :sortOrder="1"
    >
      <Column field="name" :header="__('professionaltype.table.name')" sortable frozen style="min-width: 200px">
        <template #body="{ data }">
          {{ data.name }}
        </template>
      ...
      <Column field="code" :header="__('professionaltype.table.code')" sortable style="min-width: 200px">
        <template #body="{ data }">
          {{ data.code }}
        </template>
        <template #filter="{ filterModel }">
      ...
      <Column :exportable="false" style="max-width: 130px">
        <template #body="slotProps">
          <Link :href="route('professionaltypes.show', slotProps.data.id)" v-if="canShow">
            <span class="material-symbols-rounded cursor-pointer transition-all text-slate-500 hover:text-sky-600">visibility</span>
          </Link>
          <Link :href="route('professionaltypes.edit', slotProps.data.id)" v-if="canEdit">
